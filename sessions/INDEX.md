...
| # | Priority | File | Focus |
|---|---|---|---|
| ~~1~~ | ✅ **CORE SHIPPED 2026-06-04** (build `20260604a`) | **tasks/P2-T046-full-state-cross-device-sync.md** | **Full-state sync built + pushed: snapshotAll/restoreAll + sync wrappers, restore-on-signin, re-sync on key events, `getJourney` read-half, Settings Export/Restore backup, flag `FEATURES.fullSync`. ⚠️ NEEDS: (1) manual Code.gs redeploy (read path dead until then), (2) 2-browser-profile smoke test. Deferred: `mustChangePassword` forced screen + per-key merge.** |
